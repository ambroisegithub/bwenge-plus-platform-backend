...
// ✅ FIX: Import SystemType from UserSession so there is ONE shared enum.
// Previously User.ts declared its own SystemType with key BWENGEPLUS (no underscore)
// while the controller imports SystemType from UserSession where the key is BWENGE_PLUS
// (with underscore). Having two separate enums caused TypeScript to silently use
// whichever was imported in each file — unifying them prevents future confusion.
