...
export class AnswerController {
  ...
  // ==================== UNIFIED SUBMIT ANSWERS ENDPOINT ====================
    ...
    try {
      ...
      if (!assessment_id || !Array.isArray(answers) || answers.length === 0) {
        return res.status(400).json({
          success: false,
          message: "Missing required fields: assessment_id and answers array",
        });
      }
      ...
      if (enrollment_id) {
        ...
        if (!enrollment) {
          return res.status(404).json({
            success: false,
            message: "Enrollment not found or doesn't belong to user",
          });
        }
      ...
      // ==================== DETECT TYPE: ASSESSMENT OR QUIZ ====================
      ...
      if (assessment) {
        ...
        if (!quiz) {
          return res.status(404).json({
            success: false,
            message: "Assessment or Quiz not found with the provided ID",
          });
        }
      ...
      // ==================== VERIFY ENROLLMENT MATCHES COURSE ====================
      if (enrollment && enrollment.course_id !== courseId) {
        return res.status(400).json({
          success: false,
          message: "Enrollment course does not match assessment/quiz course",
        });
      }

      // If enrollment_id not provided, try to find it
      if (!enrollment && courseId) {
        ...
        console.log(
          enrollment
            ? `✅ [submitAnswers] Found enrollment automatically: ${enrollment.id}`
            : "⚠️ [submitAnswers] No enrollment found - proceeding without"
        );
      }
      ...
      for (const answerData of answers) {
        ...
        // Try to find question in assessment JSONB first
        if (!isQuiz && assessment?.questions) {
          questionData = assessment.questions.find((q: any) => q.id === question_id);
          if (questionData) {
            isObjectiveQuestion = ["MULTIPLE_CHOICE", "TRUE_FALSE"].includes(
              questionData.type
            );
        ...
        if (!questionData && isQuiz && quiz?.questions) {
          const quizQuestion = quiz.questions.find((q: any) => q.id === question_id);
          if (quizQuestion) {
            questionData = {
              id: quizQuestion.id,
        ...
        if (!questionData) {
          ...
          // Still save the answer but mark as requires manual grading
        ...
        if (isObjectiveQuestion) {
          ...
          const evaluation = AnswerController.evaluateQuestion(
            questionData,
            submittedAnswer
          );
        ...
        if (existingAnswer) {
          // ✅ FIX: Update existing answer - store question_id in column for both types
          existingAnswer.question_id = question_id; // Always store in column
          existingAnswer.answer = submittedAnswer; // Store actual answer
          existingAnswer.is_correct = isCorrect;
          existingAnswer.points_earned = pointsEarned;
          existingAnswer.time_spent_seconds = time_spent_seconds || 0;
          existingAnswer.attempt_number += 1;
          existingAnswer.is_final_submission = true;
          existingAnswer.is_graded = isGraded;

          if (isQuiz) {
            existingAnswer.quiz_id = assessment_id;
          }

          await answerRepo.save(existingAnswer);
          processedAnswers.push(existingAnswer);

          console.log(`🔄 [submitAnswers] Updated existing answer: ${existingAnswer.id}, question_id: ${question_id}`);
        } else {
          // ✅ FIX: Create new answer - use query builder to bypass FK validation for assessments
          const newAnswerData: any = {
            user_id: userId,
            assessment_id,
            quiz_id: isQuiz ? assessment_id : null,
            question_id: question_id, // ✅ Store question_id in column for both Quiz and Assessment
          ...
          // Use raw query to insert without FK constraint check for assessments
          if (!isQuiz) {
            await answerRepo.query(
              `INSERT INTO answers 
              (user_id, assessment_id, quiz_id, question_id, answer, is_correct, points_earned, time_spent_seconds, attempt_number, is_final_submission, is_graded, created_at, updated_at)
              ...
              [
                ...
                assessment_id,
                null, // quiz_id
                question_id, // ✅ Stored as string reference, not FK
            ...
            const createdAnswer = await answerRepo.findOne({
              where: { user_id: userId, assessment_id, question_id },
              order: { created_at: 'DESC' }
            });

            if (createdAnswer) {
              processedAnswers.push(createdAnswer);
            }
          } else {
            // For Quiz, use normal save (FK exists)
            const newAnswer = answerRepo.create(newAnswerData);
            await answerRepo.save(newAnswer);
            processedAnswers.push(newAnswer);
          }
      ...
      console.log(`📊 [submitAnswers] Results:`, {
        totalScore,
        totalPossiblePoints,
      ...
      // ==================== UPDATE PROGRESS (LESSON OR ASSESSMENT) ====================
      if (enrollment) {
        try {
          // Determine if this is lesson-based quiz or standalone assessment
          if (lessonId) {
            // Lesson-based quiz/assessment - update lesson progress
            let progress = await progressRepo.findOne({
              where: {
                enrollment_id: enrollment.id,
                lesson_id: lessonId,
                assessment_id: null, // Lesson progress, not assessment
              },
            });
            ...
            if (!progress) {
              progress = progressRepo.create({
                ...
                user_id: userId,
                course_id: courseId,
                is_completed: !requiresManualGrading && passed,
                completion_percentage: !requiresManualGrading ? percentage : 0,
                score: !requiresManualGrading ? percentage : null,
                status: !requiresManualGrading
            ...
            // Standalone assessment - create assessment progress record
            let progress = await progressRepo.findOne({
              where: {
                enrollment_id: enrollment.id,
      ...
      if (enrollment) {
        try {
          ...
          if (course) {
            ...
            // Count completed lessons (progress records with lesson_id only)
            const completedLessonsCount = await progressRepo.count({
              where: {
                enrollment_id: enrollment.id,
                is_completed: true,
                lesson_id: Not(null),
      ...
      res.json({
        ...
        message: requiresManualGrading
          ...
          : `${assessmentType} submitted successfully`,
        data: {
          assessment_id,
          ...
          answers_count: processedAnswers.length,
          objective_questions: objectiveQuestionsCount,
          subjective_questions: subjectiveQuestionsCount,
